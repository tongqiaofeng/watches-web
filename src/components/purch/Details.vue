          ...
          <div v-if="detailsSelect.num == 0" class="watch-info">
            <div class="info-top">
              <div class="info-top-main">
                <p style="width: 85px;margin: 0;">采购日期：</p>
                <div class="main-container" style="width: 100px;">
                  {{buyDate}}
                </div>
              </div>
              <div v-show="storeId > 0" style="display: flex;">
                <div class="store-name">
                  <p style="width: 85px;margin: 0;">店铺名称：</p>
                  <div>
                    {{storeName}}
                  </div>
                </div>
                <div class="store-name">
                  <p style="width: 85px;margin: 0;">详情地址：</p>
                  <div>
                    <span>{{storeAddr}}</span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        paystate: 0,
        drawbackstate: 0,
        commissionstate: this.detailsSelect.buyCommState, // 佣金状态
        tipstate: 0,
    ...
    methods: {
      ...
      detailState() {
        ...
        this.$axios.post(this.$store.state.baseUrl + '/BuyOrderGet?java', {
          ...
          this.paystate = res.data.pay.buyPayState;
          this.drawbackstate = res.data.tax.buyTaxState;
          this.tipstate = res.data.fee.buyFeeState;
          this.buyDate = res.data.buyDate;
