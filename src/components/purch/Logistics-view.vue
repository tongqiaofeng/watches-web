<template>
  <div class="view-container" id="container">
    <!-- <h1>物流</h1> -->
    <div
      v-show="logisticsViewList.length == 0"
      ref="hello"
    ...
    <div v-if="logisticsViewList.length !== 0">
      <div class="view" v-for="(item, index) in logisticsViewList" :key="index">
        <div class="purchase-row">
          ...
          <span class="purchase-number">物流单号: {{ " " + item.logId }}</span>
          <span class="purchase-date">发货时间: {{ item.logSendTime }}</span>
        </div>
        <table class="view-table">
          ...
          <tr>
            ...
            <td>{{ item.logWarehouse }}</td>
            <td>
              {{ item.logCurrency + " " + formatNumberRgx(item.logMoney) }}
            </td>
            <td>
              <div style="display: flex;justify-content:center;">
                <span>{{ item.logState == 1 ? "运输中" : "已到达仓库" }}</span>
                <div style="margin-left: 5px;">
                  <img
                    :src="item.logState == 1 ? img1 : img2"
            ...
            <td>
              ...
              <el-dialog title="详细物流信息" :visible.sync="dialogVisible">
                      ...
                      <div class="every-details">
                        ...
                        <table>
                          ...
                          >
                            ...
                            <td>
                              <div>
                                <span>{{ items.buyWatchBrand }}</span>
                              </div>
                              <div>
                                <span>{{ items.buyWatchModel }}</span>
                              </div>
                            </td>
                            ...
                            <td>
                              <!-- <span>{{items.logState == 1 ? "运输中" : "已到达仓库"}}</span> -->
                              <img
                                :src="
                                  items.logState == 2 || items.logState > 2
                                    ? img2
                                    : img1
                                "
                              />
                            ...
                            <td>
                              <div
                                style="width: 100%;margin: 0 auto;border-bottom: 1px solid #000;display: flex;"
                              >
                                <input
                                  type="text"
                                ...
                                <i
                                  slot="suffix"
                                  style="width: 50%;height: 40px;line-height: 40px;font-style:normal;color: #000;"
                                  >{{ details.logCurrency }}</i
                                >
                              </div>
                            </td>
                ...
                <div slot="footer">
                  ...
                  <el-button type="primary" @click="viewClick2" v-preventClick
                    >确 定</el-button
                  >
                </div>
              </el-dialog>
...
export default {
  ...
  methods: {
    ...
    viewDetails(details) {
      ...
      this.nums = this.details.logWatch.length;
