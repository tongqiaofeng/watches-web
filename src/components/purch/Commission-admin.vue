        ...
        <div>
          ...
          >
            ...
            <tr>
              ...
              <td>
                {{
                  formatNumberRgx(item.obj.buyCommMoney) +
                    " " +
                    (item.type == 0
                      ? item.obj.buyTaxCurrency
                      : item.obj.buyCommCurrency)
                }}
              </td>
              <td>
                <!-- <el-button type="text" @click="checkDetails(item.obj.buyId)">查看详情</el-button> -->
                <el-tooltip
                  class="item"
                  effect="light"
                  content="查看详细信息"
...
export default {
  ...
  methods: {
    ...
    handleCommission() {
      this.msg = "数据加载中...";
      this.$axios
        .post(this.$store.state.baseUrl + "/BuyOrderCommList?java", {
          page: this.page,
