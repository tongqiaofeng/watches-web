  ...
  <div class="watchinfo-container">
    <div class="watchinfo">
      <table>
        ...
        <tr v-for="(con, index) of buyWatch" :key="index" class="second-tr">
          <td>
            <img :src="con.buyWatchState == 1 ? watchimg1 : watchimg2" />
          </td>
          <td>
            <div>
              ...
              <img
                v-image-preview
                :src="
                  con.buyWatchPics == null || con.buyWatchPics == ''
                    ? ''
                    : img +
                      '/img/watch/' +
                      (con.buyWatchPics || '').split('|')[0]
                "
          ...
          <td>
            <div>
              ...
              <span>{{ con.buyWatchBrand }}</span>
            </div>
            <div>
              <span>{{ con.buyWatchModel }}</span>
            </div>
          </td>
          <td>{{ con.buyWatchSn }}</td>
          <td>
            {{
              con.buyWatchCurrency + " " + formatNumberRgx(con.buyWatchPrice)
            }}
          </td>
          <td>
            <el-tooltip
...
export default {
  ...
  methods: {
    ...
    obtain() {
      ...
      this.$axios
        ...
        .then((res) => {
          console.log("手表详细信息");
          console.log(res);
          this.buyWatch = res.data.watch;
          console.log(this.buyWatch);
        });
    },
    // 编辑
    editCatch(con) {
      ...
      console.log("111");
      console.log(con);
      this.buyWatchSn = con.buyWatchSn;
      this.buyWatchCard = con.buyWatchCard;
      this.accessory = con.buyWatchParts.split("|");
      this.buyWatchBand = con.buyWatchBand;
      this.id = con.id;
    },
    ...
    messageSure() {
      ...
      this.$axios
        .post(this.$store.state.baseUrl + "/BuyWatchModify", {
          id: this.id,
        ...
        .then((res) => {
          ...
          console.log(res.data.buyWatchState);
          this.$emit("listenWatchInfo", res.data.buyWatchState);
