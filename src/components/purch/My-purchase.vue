    ...
    >
      <div class="radio-select">
        ...
        <el-radio
          ...
          label="5"
          border
          @change="getBuyOrderList10"
          >已分红</el-radio
        >
      </div>
    </div>
    <div
      v-if="numSelect.select == 0"
      style="width: 100%;background-color: #fff;"
    >
      <div class="radio-select">
        <el-radio
          ...
          label="0"
          border
          @change="getBuyOrderAllList1"
          >全部订单</el-radio
        >
        <el-radio v-model="filtrate" label="1" border @change="getBuyOrderList1"
          >待处理</el-radio
        >
        <el-radio v-model="filtrate" label="2" border @change="getBuyOrderList2"
          >待发货</el-radio
        >
        <el-radio v-model="filtrate" label="3" border @change="getBuyOrderList3"
          >入库待售</el-radio
        >
        <el-radio v-model="filtrate" label="4" border @change="getBuyOrderList4"
          >待分红</el-radio
        >
        <el-radio v-model="filtrate" label="5" border @change="getBuyOrderList5"
          >已分红</el-radio
        >
      </div>
    </div>
    <div>
      <div v-if="selectSearch.msg == 0" class="mypurchase-container">
                ...
                <div>
                  ...
                  >
                    <img
                      ...
                      @click="deletePurchaseOrder(item.buyId)"
                    />
                  </el-tooltip>
                  <el-dialog
                    title="删除采购单"
                    :visible.sync="dialogDeletePurchaseOrderVisible"
                    center
                  >
                    ...
                    <div slot="footer">
                      ...
                      <el-button
                        @click="dialogDeletePurchaseOrderVisible = false"
                        >取 消</el-button
                      >
                      <el-button
                        type="primary"
                        @click="deletePurchaseOrderSure"
                        v-preventClick
                        >确 定</el-button
                      >
                    </div>
      ...
      <Shipping-order
        v-if="selectSearch.msg == 5"
        ref="shippingOrder"
      ></Shipping-order>
      <For-account
        v-if="selectSearch.msg == 3"
        ref="forAccount"
        :forAccountNum="forAccountNum"
        @shippingState="shippingState"
      >
      </For-account>
      <Have-account
        v-if="selectSearch.msg == 4"
        ref="haveAccount"
        :haveAccountNum="haveAccountNum"
        @shippingState="shippingState"
      ></Have-account>
    </div>
  </div>
    ...
    deletePurchaseOrderSure() {
      this.$axios
        .post(this.$store.state.baseUrl + "/BuyOrderDel", {
          buyId: this.deletePurchaseOrderId,
        })
        .then((res) => {
          console.log("删除采购单");
          console.log(res);
