    ...
    <div class="paypage">
      ...
      <div class="paypage-center">
        ...
        <div class="add-pay">
          ...
          <div v-show="buyPay.length !== 0" class="pay-table">
            <table class="table-container">
              ...
              <tr v-for="(pay, index) of buyPay" :key="index">
                <td>{{ pay.time }}</td>
                <td>
                  {{ pay.type == 0 ? "刷卡" : pay.type == 1 ? "现金" : "转账" }}
                </td>
                <td>
                  {{ buyWatchCurrency + " " + formatNumberRgx(pay.money) }}
                </td>
                <td>
                  <el-tooltip
                    class="item"
                  ...
                  <el-dialog title="提示" :visible.sync="dialogVisiblePay">
                    ...
                    <div slot="footer">
                      <el-button @click="dialogVisiblePay = false"
                        >取 消</el-button
                      >
                      <el-button type="primary" @click="suredelpay"
                        >确 定</el-button
                      >
                    </div>
                  </el-dialog>
...
export default {
  ...
  methods: {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          for (let item of this.watches) {
            this.prices += Number(item.buyWatchPrice);
            this.buyWatchCurrency = item.buyWatchCurrency;
          }
          this.buyPay = res.data.pay.buyPayLst;
          if (
            res.data.pay.buyPayPic !== null &&
            res.data.pay.buyPayPic !== ""
          ) {
            this.imgSrc = res.data.pay.buyPayPic.split("|");
    ...
    paySure() {
      if (this.judge() !== 1) {
        let pay = {
          ...
          type: this.type == "刷卡" ? 0 : this.type == "现金" ? 1 : 2,
          money: this.money,
          currency: this.buyWatchCurrency,
        };
        this.buyPay.push(pay);
        console.log("999999");
        console.log(this.buyPay);
        this.payed = 0;
        for (let item of this.buyPay) {
    ...
    suredelpay() {
      ...
      this.buyPay.splice(this.delpayId, 1);
    ...
    payPageSubmit() {
      ...
      console.log(sessionStorage.getItem("buyId"), this.buyPay, storeImgUrl2);
      this.$axios
        .post(this.$store.state.baseUrl + "/BuyPaySave", {
          buyId: sessionStorage.getItem("buyId"),
          buyPay: this.buyPay,
          buyPayPic: storeImgUrl2,
        })
        .then((res) => {
          ...
          this.$emit("listenPay", res.data.buyPayState);
          this.$message.success({
            message: "付款信息保存成功",
            showClose: true,
            duration: 2000,
