  ...
  <div class="stock-pending-container">
              ...
              >
                <div class="purchase-row">
                  ...
                  <span class="purchase-number"
                    >物流单号: {{ " " + item.logId }}</span
                  >
                  <span class="purchase-date"
                    >发货时间: {{ item.logSendTime }}</span
                  >
                </div>
                <table>
                  <tr>
                    <th>
                      {{ item.deliverWarehouse == "" ? "负责人" : "发货仓库" }}
                    </th>
                    <th>货物数量</th>
                    <th>预计到达时间</th>
                    <th v-if="item.companyName != ''">寄卖公司名称</th>
                    <th v-if="item.backCompanyName != ''">返还公司名称</th>
                    <th>操作</th>
                  </tr>
                  <tr>
                    ...
                    <td>
                      {{
                        item.deliverWarehouse == ""
                          ? item.nick
                          : item.deliverWarehouse
                      }}
                    </td>
                    <td>{{ item.logWatch.length }}</td>
                    <td>{{ item.logArriveTime }}</td>
                    <td v-if="item.companyName != ''">
                      {{ item.companyName }}
                    </td>
                    <td v-if="item.backCompanyName != ''">
                      {{ item.backCompanyName }}
                    </td>
                    <td>
                      <el-tooltip
                        class="item"
                ...
                <tr v-for="(items, index) in stockPending" :key="index">
                  <td>
                    ...
                    <img
                      v-image-preview
                      :src="
                        items.buyWatchPics == null || items.buyWatchPics == ''
                          ? ''
                          : img +
                            '/img/watch/' +
                            (items.buyWatchPics || '').split('|')[0]
                      "
                      style="width: 100px;height: 100px;object-fit: cover;border-radius: 30px;"
                    />
                  </td>
                  <td>{{ items.buyWatchBrand }}</td>
                  <td>{{ items.buyWatchModel }}</td>
                  <td>{{ items.buyWatchSn }}</td>
                  <td>
                    <el-button type="text" @click="stockPendingButton(items)"
                      >入库</el-button
                    >
                    <el-dialog
                      title="入库"
                    ...
                    >
                          ...
                          >
                            ...
                            <el-form-item label="手表状态：">
                              <div style="display: flex;">
                                <el-radio v-model="radioWatch" label="1"
                                  >全新</el-radio
                                >
                                <el-radio v-model="radioWatch" label="2"
                                  >二手</el-radio
                                >
                              ...
                              <div class="upload-imgs">
                                <div class="add">
                                  ...
                                  >
                                    <input
                                      @change="
                                        inputChange($event, items.buyWatchSn)
                                      "
                                      type="file"
                                      name="img"
                                      accept="image/*"
                      ...
                      <div slot="footer">
                        ...
                        <el-button
                          type="primary"
                          @click="sureStockProduct(items.id)"
                          v-preventClick
                          >确 定</el-button
                        >
                      </div>
    ...
    <div class="stock-body" v-if="stock1.num == 1">
      <Stock-pending
        ...
        @gobackStockList="gobackStockList"
      ></Stock-pending>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          if (this.stockPending.length == 0) {
            this.hintMsg = "啊哦~没有符合该条件的商品，请重新输入关键字";
          }
        })
        .catch((err) => {
          console.log(err);
    ...
    stockPendingButton(items) {
      console.log(items);
      if (items.companyName == "") {
        this.isConsignment2 = false;
      } else {
        this.isConsignment2 = true;
      }
      this.buyWatchBrand = items.buyWatchBrand;
      this.buyWatchModel = items.buyWatchModel;
      this.stockNo = items.stockNo;
      this.stockNoCrc = items.stockNoCrc;
      this.imgurls = [];
      this.radioWatch = "1";
      this.watchState = [];
      this.stockInstateA = "";
    ...
    verification() {
      ...
      if (this.watchState.length > 1) {
        this.stockInstateA = this.watchState.join("|");
      } else {
        this.stockInstateA = this.watchState[0];
      }
      console.log(this.stockInstateA);
    },
    // 模糊搜索指定的手表入库
    sureStockProduct(id) {
      console.log(id);
      if (this.verification() !== 1) {
        ...
        if (this.stockInstateA !== undefined) {
          this.stockInstateA = this.radiowatchs + "|" + this.stockInstateA;
        } else if (this.stockInstateA == undefined) {
          this.stockInstateA = this.radiowatchs;
        }
        this.$axios

          .post(this.$store.state.baseUrl + "/StockInDo", {
            id: id,
            buyUserId: this.isConsignment2 == true ? this.buyUserId : null,
            stockInstateA: this.stockInstateA,
            stockShelfNoA: this.stockShelfNoA,
            stockInPic: storeImgUrl3,
          })
          .then((res) => {
            console.log("是否入库成功");
            console.log(res);
