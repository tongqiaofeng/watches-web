    ...
    <div v-if="outboundQuery.query == 2" class="list-details">
      ...
      <div class="list-top">
        <div class="style-margin">
          <span>
            货号：
            <span>{{stockwatchinfo.stockNo}}</span>
          </span>
        </div>
        <div class="style-margin" style="display: flex;">
          <span>仓库货架号：</span>
          <span style="margin: 0 30px 0 10px;">{{stockwatchinfo.stockShelfNoA}}</span>
            ...
            <div>
              <el-form label-width="120px">
                <el-form-item label="新的货架号：">
                  <el-input v-model="stockShelfNoA"></el-input>
                </el-form-item>
              </el-form>
            </div>
            <div slot="footer">
        ...
        <div class="style-margin" v-if="stockwatchinfo == 1">
          <span>
            ...
            <span>已被 {{stockwatchinfo.sellUserNick}} 出售</span>
          </span>
        </div>
      </div>
        ...
        <div>
          <div style="margin: 30px 0;display: flex;justify-content: space-between;">
            <div style="width: 33%;display: flex;">
              ...
              <span>{{stockwatchinfo.buyWatchBrand}}</span>
            </div>
            <div style="width: 33%;display: flex;">
              <span>型号：</span>
              <span>{{stockwatchinfo.buyWatchModel}}</span>
            </div>
            <div style="width: 33%;display: flex;">
              <span>库存：</span>
              <span>{{stockwatchinfo.watchNum}}</span>
            </div>
          </div>
          <div style="display: flex;justify-content: space-between;">
            <div style="width: 33%;display: flex;">
              ...
              <span>{{stockwatchinfo.buyWatchSn}}</span>
            </div>
            <div style="width: 33%;display: flex;">
              <span>保卡日期：</span>
              <span>{{stockwatchinfo.buyWatchCard}}</span>
            </div>
            <div style="width: 33%;display: flex;">
              <span>状态：</span>
            ...
            <el-row>
              <el-col :span="12">
                <el-button
                  ...
                  @click="checkWatch(stockwatchinfo.buyWatchId)">查看与此手表同型号的未出库的手表</el-button>
              </el-col>
              <el-col :span="12">
            ...
            <div>
              <el-form label-width="100px">
                <el-form-item label="提货人：" style="text-align: left;">
                  ...
                  <el-input v-model="sellSendUserNick" placeholder="请输入提货人昵称" style="width: 60%;"></el-input>
                </el-form-item>
                ...
                <el-form-item label="销售：">
                  ...
                  <p style="text-align: left;">{{sellUserNick}}</p>
                </el-form-item>
              </el-form>
            </div>
            <div slot="footer">
        ...
        <table>
          <tr>
            ...
            <th>
              ...
              {{items.sellState == 1 ? '销售员' : (items.logState == 0 ? '采购时间' : (items.logState == 1 ? '预计到达时间' : ''))}}
            </th>
            <th>物流状态</th>
            <th>库存信息</th>
          </tr>
          <tr>
            ...
            <td>
              ...
              {{items.sellState == 1 ? items.sellUserNick : (items.logState == 0 ? items.buyDate : (items.logState == 1 ? items.logArriveTime : ''))}}
            </td>
            <td>
              {{items.sellState == 1 ? '已出售' : (items.logState == 0 ? '采购中' : (items.logState == 1 ? '运输中' : '已入库'))}}
            </td>
            <td>
              <div v-show="items.logState !== 2">
                <span style="font-size: 15px;">当前商品未入库，暂无库存信息</span>
              </div>
              <div v-show="items.logState == 2" class="td-center">
  ...
  export default {
    data() {
      return {
        ...
        sellSendUserNick: "", // 送货人/提货人昵称
        sellUserNick: "", // 销售员昵称
        sellState: 0, // 销售状态
        sellStockToken: "", // 出库验证码/提货码
        barCode: "",
        barCode3: "",
        stockNo: "", // 货号
        doing: false,
    ...
    methods: {
          ...
          if (this.barCode.length == 36) {
            this.$axios
              ...
              .then(res => {
                ...
                if (this.stockwatchinfo.stockInPic !== '' && this.stockwatchinfo.stockInPic !== null) {
                  if (this.stockwatchinfo.stockInPic.indexOf('|') !== -1) {
                    this.imgs = this.stockwatchinfo.stockInPic.split("|");
                  } else {
                    this.imgs.push(this.stockwatchinfo.stockInPic);
                  }
                }
                ...
                this.sellStockToken = this.stockwatchinfo.sellStockToken;
                this.sellState = this.stockwatchinfo.sellState;
                this.sellUserNick = this.stockwatchinfo.sellUserNick;
                this.buyWatchBrand = this.stockwatchinfo.buyWatchBrand;
                this.buyWatchModel = this.stockwatchinfo.buyWatchModel;
                this.stockNo = this.stockwatchinfo.stockNo;
                this.stockNoCrc = this.stockwatchinfo.stockNoCrc;
                this.id = this.stockwatchinfo.id;
                console.log(this.stockwatchinfo);
                console.log(this.sellUserNick);
      ...
      inventoryInfo(id) {
        this.$axios
          ...
          .then(res => {
            ...
            // 手表id  buyWatchId 用于获取同品牌未出库手表列表
            this.watchid = this.stockwatchinfo.buyWatchId;
            this.imgs = this.stockwatchinfo.stockInPic.split("|");
            this.watchState = this.stockwatchinfo.watchState.split("|");
            this.stockNo = this.stockwatchinfo.stockNo;
            this.sellStockToken = this.stockwatchinfo.sellStockToken;
      ...
      ShelfNoASure() {
        this.$axios
          .post(this.$store.state.baseUrl + "/StockShelfNoModify", {
            id: this.id,
            stockShelfNoA: this.stockShelfNoA
          })
          .then(res => {
            console.log("修改货架号");
            console.log(res);
            ...
            this.stockwatchinfo.stockShelfNoA = this.stockShelfNoA;
      ...
      */
      ...
      stockRemoval() {
        ...
        console.log(this.sellState);
        console.log(this.sellStockToken);
        this.imgurls = [];
        this.sellSendUserNick = "";
        this.dialogVisible = true;
      },
      // 确认出库
      stockRemovalSure() {

        if (this.sellSendUserNick == "") {
          this.$message.error({
            message: "提货人不能为空，请输入",
            showClose: true,
            duration: 2000
          ...
          this.$axios
            .post(this.$store.state.baseUrl + "/StockOutDo", {
              ...
              sellSendUserNick: this.sellSendUserNick,
              sellStockToken: this.barCode,
              stockOutPic: storeImgUrl5,
            })
            .then(res => {
              console.log(res);
              this.$message.success({
