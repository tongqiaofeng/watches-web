  ...
  <div class="not-outbound-container" id="not-outbound">
      ...
      <div v-if="notOutbound.select == 2" class="not-outbound-top">
        <div class="radio-select">
          ...
          <el-radio
            ...
            label="0"
            border
            @change="getStockListAllList2"
            >全部：采购中+已入库</el-radio
          >
          <el-radio v-model="filtrate1" label="1" border @change="getStockList3"
            >采购中</el-radio
          >
          <el-radio v-model="filtrate1" label="2" border @change="getStockList4"
            >已入库</el-radio
          >
        </div>
      </div>
      <div v-if="notOutbound.select == 1" class="not-outbound-top">
        <div class="radio-select">
          ...
          <el-radio
            ...
            label="0"
            border
            @change="getStockListAllList1"
            >全部：采购中+已入库</el-radio
          >
          <el-radio v-model="filtrate" label="1" border @change="getStockList1"
            >采购中</el-radio
          >
          <el-radio v-model="filtrate" label="2" border @change="getStockList2"
            >已入库</el-radio
          >
        </div>
      </div>
      <div
        v-show="notOrders.notOutboundList1.length == 0"
        style="margin-top: 80px;text-align: center;"
      >
        <p>{{ hintMsg }}</p>
      </div>
      <div
        v-if="notOrders.notOutboundList1.length !== 0"
        style="margin-top: 80px;"
      >
        <div style="margin-left: 5%;">
          <p style="font-size: 18px;">
            {{ "商品数量: " + " " + notOrders.totalNum1 }}
          </p>
        </div>
        <div class="not-container">
          <table>
            ...
            <tr
              v-for="(item, index) of notOrders.notOutboundList1"
              :key="index"
            >
              <td>
              ...
              <td>
                ...
                <p v-if="filtrate == 0">
                  采购中：{{ item | formatPurchNumRgx(0) }}
                </p>
                <p v-if="filtrate == 0">
                  已入库：{{ item | formatPurchNumRgx(2) }}
                </p>
    ...
    <div v-if="notOutbound.nots == 1">
      <Product-list
        ...
        :watchList="watchList"
        @gobackNotOutbound="gobackNotOutbound"
...
export default {
  ...
  props: ["notOutbound", "notOrders", "notoqyeryval"],
  mounted() {
    console.log("4444");
    if (this.notoqyeryval.backval == 1) {
      console.log("back1" + "page" + this.notoqyeryval.page);
      this.notOutbound.nots = 0;
      this.notOutbound.select = 1;
      this.page = this.notoqyeryval.page;
      this.filtrate = sessionStorage.getItem("tabelfiltrate");
      console.log(sessionStorage.getItem("tabelfiltrate") + this.filtrate);
      if (sessionStorage.getItem("tabelfiltrate") == 0) {
        this.getStockListAllList1();
        // this.handleCurrentChange(this.page)
        console.log("all");
      } else if (sessionStorage.getItem("tabelfiltrate") == 1) {
        // this.handleCurrentChange(this.page)
        this.getStockList1();
      } else if (sessionStorage.getItem("tabelfiltrate") == 2) {
        // this.handleCurrentChange(this.page)
        this.getStockList2();
      }
    } else {
  ...
  filters: {
    formatPurchNumRgx: function(watchArr, state) {
      ...
      for (let item of watchArr.watch) {
        ...
        if (state == 1) {
          if (state == item.sellState) {
            // console.log("销售状态" + item.sellState);
            num3++;
          }
        }
      }
  ...
  methods: {
    ...
    // 模糊搜索
    stockInSearch() {
      console.log(this.keyword + "-->page" + this.page);
      if (this.keyword !== "") {
        ...
        this.$axios
          ...
          .then((res) => {
            ...
            // this.total = 0;
            // this.totalNum = 0;
            this.notOrders.notOutbound = res.data;
    ...
    //跳转至手表详情页面
    nottoquery(val) {
      sessionStorage.setItem("tabelfiltrate", this.filtrate);
      this.$emit("nottoquery", 1, val, 0, this.page);
    },
    // 获取未出库所有商品列表
    ...
    getStockListAllList() {
      console.log(this.filtrate + this.page);
      this.hintMsg = "数据加载中...";
      this.$axios
        .post(this.$store.state.baseUrl + "/StockList", {
          page: this.page,
    ...
    // 采购中
    getStockList1() {
      console.log(this.filtrate + "getStockList1");
      this.$emit("notKeywordChan", "");
      this.notOutbound.select = 1;
      if (this.notoqyeryval.backval != 1) {
        this.page = 1;
    ...
    // 已入库
    getStockList2() {
      console.log(this.filtrate + "getStockList2");
      this.$emit("notKeywordChan", "");
      this.notOutbound.select = 1;
      if (this.notoqyeryval.backval != 1) {
        this.page = 1;
