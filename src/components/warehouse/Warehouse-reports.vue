          ...
          <div class="watch-nums">
            ...
            <div class="report-salenums">
              ...
              <p class="watch-nums-total">
                {{
                  formatNumberRgx(warehouseWatchmoney) +
                    " " +
                    warehouseWatch.currency
                }}
              </p>
            </div>
          </div>
            ...
            <div class="report-sort">
              ...
              >
                ...
                <div class="caret" v-if="updownshow == true">
                  <i class="el-icon-caret-top" v-if="dispalytype == 1"></i
                  ><i class="el-icon-caret-bottom" v-if="dispalytype == 2"></i>
                </div>
              </div>
              <div
                class="price-sort"
          ...
          <table>
            ...
            <tr v-for="(item, index) in list" :key="index" class="imgurlbox">
              ...
              <td>
                <p>
                  {{
                    formatNumberRgx(item.buyWatchPriceHkd) +
                      "  " +
                      item.currency
                  }}
                </p>
              ...
              <td v-if="item.stockInTime == null" style="text-align: left;">
                <div style="display: flex;">
                  <span>购入时长 : </span>
                  <span
                    :class="isJump == 0 ? 'my-face' : ''"
                    :style="{
                      color:
                        timefilter2(item.buyDate) > timetocolor
                          ? 'red'
                          : '#2c3e50',
                      fontSize: '18px',
                    }"
              ...
              <td v-else style="text-align: left;">
                <div>
                  ...
                  <span
                    ...
                    :style="{
                      color:
                        timefilter2(item.stockInTime) > timetocolor
                          ? 'red'
                          : '#2c3e50',
                      fontSize: '18px',
                    }"
                    >{{ timefilter2(item.stockInTime) }}{{ " " + "天" }}</span
                  >
                </div>
                <div>
                  <span>入库时间 : </span>
                  <span>{{
                    String(item.stockInTime.split(" ").splice(0, 1))
                  }}</span>
                </div>
              </td>
              <td class="salebox">
        ...
        <div class="message-content">
          ...
          <div class="message-main">
            <div class="message-mainbox">
              ...
              >
                <span>品牌 ：</span
                ><span>{{ detailstitem.buyWatchBrand }}</span>
              </div>
              <div
                style="display: flex;flex-wrap: nowrap;"
                class="mainbox-line"
              >
                <span>型号 ：</span
                ><span>{{ detailstitem.buyWatchModel }}</span>
              </div>
            </div>
            <div class="message-mainbox">
              <div
              ...
              >
                <span>采购价格 ：</span
                ><span>{{
                  formatNumberRgx(detailstitem.buyWatchPriceHkd) +
                    " " +
                    detailstitem.currency
                }}</span>
              </div>
              <div
                style="display: flex;flex-wrap: nowrap;"
                class="mainbox-line"
              >
                <span>采购时间 ：</span><span>{{ detailstitem.buyDate }}</span>
              </div>
              <div
                style="display: flex;flex-wrap: nowrap;"
                class="mainbox-line"
                v-if="detailstitem.stockInTime == null"
              >
                <span>购入时长 ：</span
                ><span
                  >{{ timefilter2(detailstitem.buyDate) }}{{ " " + "天" }}</span
                >
              </div>
            </div>
            <div
              class="message-mainbox"
              v-if="detailstitem.stockInTime != null"
            >
              <div
                style="display: flex;flex-wrap: nowrap;"
              ...
              >
                <span>仓库名称 ：</span
                ><span>{{ detailstitem.warehouseName }}</span>
              </div>
              <div
                style="display: flex;flex-wrap: nowrap;"
                class="mainbox-line"
              >
                <span>入库时间 ：</span
                ><span>{{ detailstitem.stockInTime }}</span>
              </div>
              <div
                style="display: flex;flex-wrap: nowrap;"
                class="mainbox-line"
              >
                <span>在库时长 ：</span
                ><span
                  >{{ timefilter2(detailstitem.stockInTime)
                  }}{{ " " + "天" }}</span
                >
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    wareList() {
      ...
      if (this.checked1 == true && this.checked2 == true) {
        ...
        this.houseList.forEach((item) => {
          if (item.stockInTime == null) {
            ...
            this.warehouseWatchmoney =
              item.buyWatchPriceHkd + this.warehouseWatchmoney;
          }
        });
      ...
      //计算总金额
      this.houseList.forEach((item) => {
        let aDate = (item.stockInTime == null
          ? item.buyDate
          : String(item.stockInTime).split(" ")[0]
        ).split("-");
        let oDate2 = new Date(aDate[0], aDate[1], aDate[2]);
        if (
          parseInt(Math.abs(this.oDate1 - oDate2) / 1000 / 60 / 60 / 24) >=
          this.timetocolor
        ) {
          this.warehousenum++;
        }
    ...
    getList() {
      ...
      this.list = this.houseList.filter(
        (item, index) =>
    ...
    warehousetime(time) {
      ...
      this.warehouseWatch.watchList.forEach((item) => {
        ...
        let oDate2 = new Date(aDate[0], aDate[1], aDate[2]);
        if (
          parseInt(Math.abs(this.oDate1 - oDate2) / 1000 / 60 / 60 / 24) >= time
        ) {
