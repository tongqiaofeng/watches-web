<template>
  <div class="enterprise" id="enterprise">
    <div class="enterprisemessage">
      <div style="line-height: 72px;font-size: 25px;">
        {{ manageIndexlist.name }}
      </div>
    </div>
    <div class="enterprisehead">
      <div class="deptandstaff">
        <div
          class="deptandstaffbutton"
          :style="{ backgroundColor: rotate == true ? '#d7ebe7' : '#fff' }"
          @click="deptListclick"
        >
          <span class="deptandstaffbutton1">部门职位管理</span>
          <img
            src="../../assets/imgs/right.png"
            class="deptandstaffimg"
            alt=""
            :class="[rotate ? 'go' : 'aa']"
          />
        </div>
        <div
          class="deptandstaffbutton"
          @click="staffListclick"
          :style="{ backgroundColor: rotate2 == true ? '#d7ebe7' : '#fff' }"
        >
          <span class="deptandstaffbutton1">员工管理</span>
          <span v-if="rotate2 == false">{{
            manageIndexlist.staffNum + " " + "人"
          }}</span>
          <span v-else>{{ " " }}</span>
          <img
            src="../../assets/imgs/right.png"
    ...
    <div>
      <div v-show="enterprisenum == 0">
        <div class="deptboby">
          ...
          >
            ...
            <el-button
              @click="adddeptnum"
              style="background-color: #d7ebd7;margin-right: 30px;"
              >增加部门
            </el-button>
          </div>
          <div style="margin-top: 20px;" v-if="total > 0">
            <table class="table">
              ...
              <tr
                v-for="(item, index) in deptshow.list"
                :key="index"
                class="table-td"
              >
                <td>
                ...
                <td>
                  <img
                    ...
                    alt=""
                    title="修改该部门信息"
                    @click="changedeptmessage(item.id)"
                    style="cursor: pointer;margin:0 20px"
                  />
                  <img
                    src="../../assets/imgs/myNot.png"
                    title="删除该部门信息"
                    @click="Deletedeptmessage(item.id)"
          ...
          <div
            v-else
            style="background-color: #fff;border-radius: 30px;height: 100px;"
          >
      ...
      <div v-if="enterprisenum == 3">
        <div class="returnlastpage" @click="backto(1)">
          ...
          <span
            style="font-weight: bold;line-height: 30px;color: #919090;font-size: 18px;"
            >返回</span
          >
        </div>
        <div class="deptmessage">
          <div style=" padding: 20px 0;">
            <div style="font-size: 18px;margin-left:9%;font-weight: bold;">
              部门信息 ：
            </div>
            <div class="adddeptmessagebox">
              <el-form style="width:80%;margin: 20px auto;">
                <div style="display: flex;justify-content: space-between;">
                  ...
                  <el-button
                    v-if="enterprisenumchildren == 4"
                    @click="deptSave"
                    class="mouseoverchange"
                    >修改!
                  </el-button>
                  <el-button
                    v-if="enterprisenumchildren != 4"
              ...
              >
                ...
                <div style="display: flex;justify-content: space-around;">
                  <div class="adddeft-staff">
                    <span style="margin: 0 10px;">职位 : </span
                    ><span>{{ deptandjoblist.length + " " + "个" }}</span>
                  </div>
                  <div class="adddeptbox" @click="addjobnum">
                    <img
                      ...
                      class="adddeptimg"
                    />
                    <span style="margin-left: 10px;color: #919090;"
                      >增加职位</span
                    >
                  </div>
                </div>
                ...
                <div style="display: flex;flex-wrap: wrap;">
                  <div
                    v-for="(item, index) in deptandjoblist"
                    :key="index"
                    style="background-color: #f3fbf9;margin: 20px 0 0 20px; width: 30%;border-radius: 10px;"
                  >
                    <div
                      style="margin: 20px 20px 16px;text-align: left;font-weight: bold;font-size: 18px;"
                    >
                      {{ item.jobName }}
                    </div>
                    <div style="text-align: left;">
                      <span style="margin: 0 10px 0px 20px;font-size: 18px;">{{
                        item.total + " " + "人"
                      }}</span
                      ><span style="color: #919090;">在职</span>
                    </div>
                    <div style="margin: 20px 0;display: flex;">
                      <div
                        @click="changejobmessage(item.id)"
                        style="font-size: 14px;cursor: pointer;margin:0 20px;"
                      >
                        <img
                          src="../../assets/imgs/update.png"
                          title="修改该职位信息"
      ...
      <div v-if="enterprisenum == 5">
        ...
        <div class="addstaffmessage">
          <!-- 修改职位 -->
          <div
            v-show="enterprisenumchildren == 6 || enterprisenumchildren == 7"
            style="display: flex;justify-content: space-around; height: 50px; line-height: 50px;padding-top: 20px;"
          >
            <div
              class="changjobbutton"
              :style="{
                backgroundColor: isjobbutton == true ? '#fff' : '#d7ebe7',
              }"
            ...
            <div
              class="changjobbutton"
              :style="{
                backgroundColor: isjobbutton == false ? '#fff' : '#d7ebe7',
              }"
          ...
          <div style=" padding: 20px 0;" v-if="showjobauthority == 1">
            ...
            <div class="addstaffmessagebox">
              ...
              <div style="width:58%;margin: 0 auto;">
                <div class="addstffbox">
                  ...
                  <div
                    ...
                    v-for="items in authorityGroupList"
                    :key="items.id"
                  >
                    ...
                    <div
                      style="display: flex;flex-wrap: wrap; font-size: 16px;margin-left: 8px;"
                    >
                      <el-checkbox-group v-model="jurisdictionjobarr">
                        <el-checkbox
                          v-for="list in authorityList"
                          :label="list.id"
                          @change="depetjobchange"
                          v-show="list.groupId == items.id"
                          :key="list.id"
                          >{{ list.name }}
                        </el-checkbox>
                      </el-checkbox-group>
              ...
              <div style="margin: 4%;">
                <el-button class="savebutton" @click="jobsavebutton"
                  >保存</el-button
                >
              </div>
            </div>
          ...
          <div v-else style=" padding: 20px 0;">
            <div class="addstaffmessagebox">
              ...
              <table
                ...
                v-if="deptjobstaff.staffList.length !== 0"
              >
                <tr>
                  <th>员工昵称</th>
                  <th>在职状态</th>
                ...
                <tr
                  v-for="(item, index) in deptjobstaff.staffList"
                  :key="index"
                ...
                >
                  ...
                  <td>
                    <span v-if="item.flag == 1">在职</span
                    ><span v-else>离职</span>
                  </td>
                  <td @click="changestaffmsg(item.id)" style="cursor: pointer;">
                    <img
                      src="../../assets/imgs/update.png"
                      alt=""
                      title="修改员工信息"
      ...
      <div class="changetable" v-if="enterprisenum == 1">
        <div class="bobychecked" id="bobychecked">
          ...
          <div style="width: 48%;display: flex;justify-content: flex-end;">
            <span
              style="line-height:40px ;    font-size: 14px;color: #919090;"
              >{{ "员工人数 : " + " " + Staff.total + "人" }}</span
            >
            <img
          ...
          <table class="table">
            ...
            >
              ...
              <td v-if="item.deptList.length > 0">
                <span
                  v-for="(items, indexs) in item.deptList"
                  :key="indexs"
        ...
        <div
          ...
          style="background-color: #fff;border-radius: 30px;height: 100px;"
        >
          <div style="margin:40px auto;text-align: center;line-height: 100px;">
            {{ msg }}
          </div>
        </div>
        <!-- 分页 -->
            ...
            <div class="addstaffmessagebox">
              <el-form style="width:58%;margin: 20px auto;">
                ...
                <div style="width: 100%;text-align: left;">
                  ...
                  <span style="margin-right: 30px;font-size: 18px;" required>
                    在职状态 :
                  </span>
              ...
              <div style="width:58%;margin: 0 auto;">
                <div class="addstffbox">
                  ...
                  <div v-for="(item, index) in deptListdata" :key="index">
                    ...
                    <div class="position">
                      <div
                        v-for="(jobnames, num) in item.jobList"
                        :key="num"
                ...
                <div class="addstffbox">
                  ...
                  >
                    ...
                    <div
                      style="display: flex;flex-wrap: wrap; font-size: 16px;margin-left: 20px;"
                    >
              ...
              <div style="margin: 4%; ">
                <el-button class="savebutton" @click="savebutton"
                  >保存</el-button
                >
              </div>
            </div>
...
export default {
  ...
  mounted() {
    this.Warehouse();
    this.authorityGroupList = JSON.parse(
      sessionStorage.getItem("authorityGroupList")
    );
    console.log("权限列表");
    console.log(this.authorityGroupList);
  ...
  methods: {
    mouseOver() {
      if (this.enterprisenumchildren == 4) {
        document.getElementsByClassName(
          "mouseoverchange"
        )[0].style.backgroundColor = "#0C860C";
      }
    },
    mouseLeave() {
    ...
    manageIndex() {
      this.$axios
        .post(this.$store.state.baseUrl + "/manageIndex")
        .then((res) => {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          if (this.deptid == "") {
            ...
            this.deptid !== "" &&
            this.deptnametemp !== this.deptname
          ) {
            this.enterprisenum = 0;
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          this.authorityList.forEach((element) => {
            if (this.deptjobstaff.authority.split("|")[element.id - 1] == 1) {
              //进入修改页面，展示原有的权限
    ...
    jobsavebutton() {
      ...
      console.log(
        this.deptid + "=====(" + this.jobid + ")=====" + this.jobname
      );
    },
    deletejobmessage(val) {
      //删除该职位信息
      this.$axios
    ...
    deptList(val) {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          if (this.deptListjosn.flag == 1) {
            //根据flag显示职位状态
            this.radio = "1";
          } else if (this.deptListjosn.flag == 0) {
            this.radio = "0";
          }
          if (this.deptListjosn.warehouseId == 0) {
            //没有权限不展示仓库
            this.showwarehouseList = false;
          } else {
          ...
          console.log(
            "/staffInfo员工列表" + this.isstaff + "--" + this.isjobnamefun
          );
    ...
    stafffunction(val) {
      //员工职位-权限显示
      if (this.isstaff == true && this.isjobnamefun == false) {
        //修改员工信息
        console.log("修改员工信息");
          ...
          for (let i = 0; i < this.deptListdata.length; i++) {
            for (let j = 0; j < this.deptListdata[i].jobList.length; j++) {
              for (let k = 0; k < this.deptListjosn.jobIdList.length; k++) {
                if (
                  this.deptListdata[i].jobList[j].id ==
                  this.deptListjosn.jobIdList[k]
                ) {
                  //展示员工职位
                  this.arryauthority.push(
                    this.deptListdata[i].jobList[j].authority
                  ); //拿到权限对象数组
                }
              }
            }
      ...
      } else if (this.isstaff == false) {
        //增加员工信息
        console.log(
          this.jobnamearr.length + "{%%%%%%%%%%%%%%%%%%%%%" + this.isjobnamefun
        );
        console.log(this.deptval);
        console.log(this.authorityList);
        if (this.deptback == true) {
          //固定职位状态
    ...
    jobnamefun() {
      ...
      for (let i = 0; i < this.deptListdata.length; i++) {
        for (let j = 0; j < this.deptListdata[i].jobList.length; j++) {
          if (
            this.jobnamearr.indexOf(this.deptListdata[i].jobList[j].id) !== -1
          ) {
            //判断职位，拿出该职位的权限
            console.log("有" + "-----" + i + j);
      ...
      console.log(this.deptval);
      if (
        this.deptval.indexOf(20 || 21 || 22) == -1 ||
        this.deptval.length == 0
      ) {
    ...
    Deletestaff(val) {
      ...
      this.$axios
        ...
        .then((res) => {
          console.log(res.data);
          this.message(1, "删除成功");
          if (parseInt(this.Staff.total / 10) + 1 == this.page) {
            //防止空显示
