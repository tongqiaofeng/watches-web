    ...
    <div v-show="completedRecordList.length !== 0">
      <table v-for="(item,index) in list" :key="index" class="borrow-table">
        ...
        <tr>
          ...
          <td>{{item.time}}</td>
          <td>
            <p :style="{color: item.type == 2 ? '#0c7063' : 'red'}">
              {{item.type == 2 ? '+' +formatNumberRgx(item.money) + ' ' +item.currency : '-' +formatNumberRgx(item.money) + ' ' +item.currency}}
            </p>
          </td>
          <td>
              ...
              <div>
                <div class="un-handle-msg">
                  ...
                  <div class="msg-main">
                    <p>申请信息：</p>
                    <div class="main-message">
                      <p><span>时间：</span><span>{{completedHandleRecord.obj.payTime}}</span></p>
                      <div style="display: flex;">描述：<p v-html="completedHandleRecord.obj.describe"
                          style="margin-top: 0;">
                        </p>
                      </div>
                    </div>
