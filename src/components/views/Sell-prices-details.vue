    ...
    <div class="details-bottom" v-if="detailsList.length !== 0">
      ...
      <table>
        ...
        <tr v-for="(item, index) in detailsList" :key="index">
          <td>
            {{
              companyLoginCurrency + " " + formatNumberRgx(item.convertPrice)
            }}
          </td>
          <td>{{ item.currency + " " + formatNumberRgx(item.price) }}</td>
          <td>
...
export default {
  ...
  created() {
    ...
    if (this.detailsList.length > 1) {
      for (let item of this.detailsList) {
        ...
        this.yList.push(item.convertPrice);
      }
    }
    this.xList.reverse();
    this.yList.reverse();
  ...
  methods: {
    tendency2(i) {
      console.log("比较大小");
      if (
        this.detailsList[i].convertPrice > this.detailsList[i + 1].convertPrice
      ) {
    ...
    judgePrice() {
      ...
      for (let price of this.detailsList) {
        this.priceList.push(price.convertPrice);
      }
