<template>
  <div>
    ...
    <div v-if="unPaySelect.select == 1">
      ...
      <div>
        <p
          :style="{ color: unPayBonusDetails.allMoney > 0 ? '#0c7063' : 'red' }"
          class="text-style"
        >
          {{
            formatNumberRgx(unPayBonusDetails.allMoney) +
              " " +
              unPayBonusDetails.currency
          }}
        </p>
      </div>
      <div>
        <p>
          分红单号：<span>{{ unPayBonusDetails.orderNo }}</span>
        </p>
        <p>
          时间：<span>{{ unPayBonusDetails.time }}</span>
        </p>
      </div>
      <div
        class="user-list-style"
        v-for="(user, index) in unPayBonusDetails.userList"
        :key="index"
      >
        <div v-if="user.type == 1 || user.type == 2">
          ...
          <div class="every-list-style">
            ...
            <el-button
              ...
              v-show="user.state == 0"
              @click="
                financeBonusCheck(user.userId, user.type, user.bonusMoney)
              "
              >确认打款</el-button
            >
            <el-dialog
              title="确认打款"
              :visible.sync="dialogFinanceBonusCheckVisible"
              center
            >
              ...
              <div slot="footer">
                <el-button @click="dialogFinanceBonusCheckVisible = false"
                  >取 消</el-button
                >
                <el-button type="primary" @click="financeBonusCheckSure"
                  >确 定</el-button
                >
              </div>
            </el-dialog>
        ...
        <div v-if="user.type == 3">
          ...
          <p>
            成本金额：<span>{{
              formatNumberRgx(user.cost) + " " + unPayBonusDetails.currency
            }}</span>
          </p>
          <p>
            分红金额：<span>{{
          ...
          <p v-show="user.state == 0">
            可抵扣债务金额：<span>{{
              formatNumberRgx(user.unDeductMoney) +
                " " +
                unPayBonusDetails.currency
            }}</span>
          </p>
          <p>
            债务抵扣金额：<span v-show="user.state !== 0">{{
              user.deductMoney
            }}</span>
            <el-input
          ...
          <div class="every-list-style">
            ...
            <el-button
              ...
              style="width: 110px;height: 40px;font-size: 14px;"
              v-show="user.state == 0"
              @click="financeBonusCheck2(user.userId, index, 1)"
              >确认已打款</el-button
            >
            <el-dialog
              title="确认打款"
        ...
        <div v-if="user.type == 4">
          ...
          <div class="every-list-style">
            ...
            <el-button
              ...
              style="width: 110px;height: 40px;font-size: 14px;"
              v-show="user.state == 0"
              @click="financeBonusCheck2(user.userId, index, 2)"
              >确认已打款</el-button
            >
            <el-dialog
              title="确认打款"
  ...
  methods: {
    ...
    deductMoneyChange(cost, bonusMoney, index) {
      ...
      if (
        ...
        Number(this.unPayBonusDetails.userList[index].cost) +
          Number(this.unPayBonusDetails.userList[index].bonusMoney)
      ) {
        this.$message.warning({
          message: "债务抵扣金额不能大于成本与分红之和，请重新输入",
          showClose: true,
          duration: 2000,
        });
      } else {
        this.unPayBonusDetails.userList[index].payMoney =
          this.shouldMoney - this.unPayBonusDetails.userList[index].deductMoney;
      }
    },
    // 确认已打款
      ...
      if (
        ...
        this.params = {
          ...
          deductMoney:
            this.unPayBonusDetails.userList[index].deductMoney == ""
              ? 0
              : Number(this.unPayBonusDetails.userList[index].deductMoney),
    ...
    financeBonusCheckSure() {
      ...
      this.$axios
        .post(
          this.$store.state.baseUrl + "/financeBonusCheck?java",
          this.params
        )
        .then((res) => {
          console.log("确认打款");
    ...
    getList() {
      ...
      this.list = this.unPayList.filter(
        (item, index) =>
