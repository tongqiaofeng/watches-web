    ...
    <div v-show="debtCompanySelect.select == 0">
      ...
      <div v-else>
        ...
        <div class="debt-person">
          ...
          >
            <span>{{
              companyAllDebt.allMoney > 0
                ? formatNumberRgx(companyAllDebt.allMoney) + " "
                : formatNumberRgx(companyAllDebt.allMoney).split("-")[1] + " "
            }}</span>
            <span>{{ companyAllDebt.currency }}</span>
          </p>
          <p style="color: #c0c0c0;">
            {{ companyAllDebt.allMoney > 0 ? "待收款" : "待还款" }}
          </p>
        </div>
...
export default {
  ...
  methods: {
    ...
    allDebt() {
      this.$axios
        .post(this.url + "/companyStatementList?java", {
          flag: 2,
        })
        .then((res) => {
          console.log("总债务列表");
