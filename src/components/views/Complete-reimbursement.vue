      ...
      <div v-if="completeReimbursementSel.select == 0">
        <table>
          ...
          <tr v-for="(item,index) in toConfirmedList" :key="index">
            ...
            <td>{{formatNumberRgx(item.checkMoney) + ' ' + item.settleCurrency}}</td>
            <td>{{item.subTime}}</td>
            <td>
        ...
        <div class="details-main">
          ...
          <table v-for="(reim,index) in toConfirmedDetails.data" :key="index">
            ...
            <tr>
              ...
              <td>{{formatNumberRgx(reim.obj.sysCheckMoney) + ' ' + toConfirmedDetails.settleCurrency}}</td>
                  ...
                  <div>
                    ...
                    <div style="margin-top: 20px;border-top: 1px solid #ddd;">
                      ...
                      <div style="padding-left: 15px;display: flex;justify-content: space-between;">
                        ...
                        <img src="../../assets/imgs/calc2.png" style="height: 35px;cursor: pointer;" @click="jumpRate(unJourneyInfo.rateTime)" />
                      </div>
                  ...
                  <div>
                    ...
                    <div style="margin-top: 20px;border-top: 1px solid #ddd;">
                      ...
                      <div style="padding-left: 15px;display: flex;justify-content: space-between;">
                        ...
                        <img src="../../assets/imgs/calc2.png" style="height: 35px;cursor: pointer;" @click="jumpRate(recordDetailsMsg.rateTime)" />
                      </div>
                      <p style="margin-top: 0;padding-left: 15px;">
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            console.log("----------------------" + this.completeReimbursementSel.val);
            if (this.completeReimbursementSel.val == 12) {
                console.log(this.completeReimbursementSel.val + "----------------------");
                this.handleConfirmedDetails(sessionStorage.getItem('4bxNumber'))
                if (this.completeReimbursementSel.select == 1) {
                    console.log(this.completeReimbursementSel.select);
                    this.lookOver(sessionStorage.getItem('4type'), sessionStorage.getItem('4id'))
                } else if (this.completeReimbursementSel.select == 2) {
                    console.log(this.completeReimbursementSel.select);
                    this.lookOver(sessionStorage.getItem('4type'), sessionStorage.getItem('4id'))
                    this.recordDetails(JSON.parse(sessionStorage.getItem('4every')))
                }
            }
        },
        methods: {
            ...
            handleConfirmedDetails(bxNumber) {
                sessionStorage.setItem('4bxNumber', bxNumber)
                this.checked = false;
                this.reimNumber = '';
            ...
            // 获取行程/非行程详细信息
            lookOver(type, id) {
                sessionStorage.setItem('4type', type)
                sessionStorage.setItem('4id', id)
                console.log(type, id);
                if (type == 0) {
            ...
            // 查看行程信息记录详情
            recordDetails(every) {
                sessionStorage.setItem('4every', JSON.stringify(every))
            ...
            jumpRate(time) {
                console.log('跳转');
                sessionStorage.setItem('completeReimbursementSel', this.completeReimbursementSel.select)
                this.$emit('rateCalcJump', 11, time, 1);
            },
            // 分页
