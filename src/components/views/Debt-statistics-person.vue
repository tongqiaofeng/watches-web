<template>
  <div class="debt-statistics-container">
    ...
    <div v-show="debtPersonSelect.select == 0">
      ...
      <div v-else>
        ...
        <div class="debt-person">
          ...
          >
            <span>{{
              personAllDebt.allMoney > 0
                ? formatNumberRgx(personAllDebt.allMoney) + " "
                : formatNumberRgx(personAllDebt.allMoney).split("-")[1] + " "
            }}</span>
            <span>{{ personAllDebt.currency }}</span>
          </p>
          <p style="color: #c0c0c0;">
            {{ personAllDebt.allMoney > 0 ? "待收款" : "待还款" }}
          </p>
        </div>
        <div class="person-table">
          <table>
            ...
            <tr v-for="(item, index) in personAllDebt.list" :key="index">
              ...
              <td>
                <span :style="{ color: item.money > 0 ? '#0c8563' : 'red' }"
                  >{{ item.money > 0 ? "待收：" : "待还："
                  }}{{ Math.abs(item.money) + " " }}</span
                >
                <span style="font-size: 12px;">{{
                  personAllDebt.currency
                }}</span>
              </td>
              <td>
                <el-tooltip
                ...
                >
                  <img
                    ...
                    @click="personDetails(item.userId)"
                    v-preventClick
                  />
                </el-tooltip>
              </td>
    ...
    <div v-show="debtPersonSelect.select == 1">
      ...
      <Debt-person-bill
        ref="debtPersonAllBill"
        :completedList="singleList"
      ></Debt-person-bill>
    </div>
    <div v-show="debtPersonSelect.select == 2">
      ...
      <div class="debt-person">
        <span :style="{ color: singleDebt.allMoney > 0 ? '#0c8563' : 'red' }">
          ...
          <span>{{ Math.abs(singleDebt.allMoney) + " " }}</span>
        </span>
        <span style="font-size: 12px;">{{ singleDebt.currency }}</span>
      </div>
      <div class="person-table">
        <div
          v-for="(item, index) in singleDebt.list"
          :key="index"
          class="table-every"
        >
          <div></div>
          <div
            :style="{ color: item.money > 0 ? '#0c8563' : 'red' }"
            class="every-main"
          >
            <div>
              {{ item.money > 0 ? "待收：" : "待还：" }}
            ...
            <div class="main-money">
              <p
                :style="{ lineHeight: item.moneyHkd == null ? '40px' : '20px' }"
              >
                {{ Math.abs(item.money) + " " + item.currency }}
              </p>
              <p style="margin-top: 5px;" v-if="item.moneyHkd !== null">
                {{ Math.abs(item.moneyHkd) + " " + singleDebt.currency }}
              </p>
            </div>
          </div>
          <div class="person-table-img">
            <img
              ...
              @click="getSingleBill(item.currency)"
              v-preventClick
            />
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    allDebt() {
      this.$axios
        .post(this.url + "/singleStatementList?java", {
          flag: 2,
        })
        .then((res) => {
          console.log("总债务列表");
