    ...
    <el-form label-width="120px">
      ...
      <el-form-item label="银行卡号：" required v-show="type !== '现金'">
        <div style="width: 100%;display: flex;">
          ...
          <el-select v-model="bankId" filterable placeholder="请选择" style="width: 60%;">
            <el-option v-for="item in bankLists" :key="item.id" :label="item.bankName + ' ' + item.bankAccount"
              :value="item.id">
            </el-option>
          </el-select>
...
<script>
  export default {
    ...
    mounted() {
      ...
      this.bankLists = this.recordListData.bankList;
    },
    computed: {
      currencyList() {
        let list = [];
    ...
    methods: {
      ...
      borrowRecordAdd() {
        if (this.type == '银行卡') {
          ...
          if (this.subMoney == '' || this.currency == '' || this.bankId ==
            '' || this.describe == '') {
            this.$message.error({
              message: '数据不能为空，请检查数据填写',
              showClose: true,
