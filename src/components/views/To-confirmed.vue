...
<script>
  export default {
    ...
    mounted() {
      ...
      console.log("----------------------" + this.toConfirmedSel.val);
      if (this.toConfirmedSel.val == 12) {
        console.log(this.toConfirmedSel.val + "--------------------");
        this.handleConfirmedDetails(sessionStorage.getItem('ToconfirmedbxNumber'))
        if (this.toConfirmedSel.select == 1) {
          console.log(this.toConfirmedSel.select);
          this.lookOver(sessionStorage.getItem('toconfirmedtype'), sessionStorage.getItem('toconfirmedid'))
        } else if (this.toConfirmedSel.select == 2) {
          console.log(this.toConfirmedSel.select);
          this.lookOver(sessionStorage.getItem('toconfirmedtype'), sessionStorage.getItem('toconfirmedid'))
          this.recordDetails(JSON.parse(sessionStorage.getItem('toconfrimedevery')))
        }
      }
    },
    methods: {
      ...
      handleConfirmedDetails(bxNumber) {
        sessionStorage.setItem('ToconfirmedbxNumber', bxNumber)
        this.checked = false;
        this.reimNumber = '';
      ...
      // 获取行程/非行程详细信息
      lookOver(type, id) {
        sessionStorage.setItem('toconfirmedtype', type)
        sessionStorage.setItem('toconfirmedid', id)
        console.log(type, id);
        if (type == 0) {
      ...
      // 查看行程信息记录详情
      recordDetails(every) {
        sessionStorage.setItem('toconfrimedevery', JSON.stringify(every))
      ...
      jumpRate(time) {
        console.log('跳转' + time);
        sessionStorage.setItem('toConfirmedSelselect', this.toConfirmedSel.select)
