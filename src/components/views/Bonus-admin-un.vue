  ...
  <div>
          ...
          <td>
            ...
            >
              <img
                ...
                style="width: 19px;height: 22px;cursor: pointer;"
    ...
    <div v-if="unBonusSelect.select == 1" class="two">
      ...
      <div>
        <p>可结算手表列表：</p>
        <div
          v-for="(watch, index) in singleBonus"
          :key="index"
          class="watch-table"
        >
          <div>
            <input
              class="selBtn"
              type="checkbox"
              :checked="watch.isNoRate == 1 ? false : true"
              :value="watch.id"
              @change="checkedChange($event, watch, index)"
              :disabled="watch.isNoRate == 1 ? true : false"
            />
          </div>
          <div>
            <div style="display: flex;">
              ...
              <div>
                <p class="p-style">
                  <span
                    >品牌：<span>{{ watch.watchBrand }}</span></span
                  >
                </p>
                <p class="p-style">
                  <span
                    >型号：<span>{{ watch.watchModel }}</span></span
                  >
                </p>
                <p class="p-style">
                  <span
                    >等级：<span>{{ watch.watchLevel }}</span></span
                  >
                </p>
                <p class="p-style">
                  <span
                    >批发价：<span>{{ watch.price }}</span></span
                  >
                </p>
              </div>
            ...
            <div style="display: flex;">
              <div>
                <p>
                  售价：<span style="color: #0c7063;">{{
                    formatNumberRgx(watch.sellMoney) + " " + watch.currency
                  }}</span>
                </p>
                <p>
                  实际成本：<span>{{
                    formatNumberRgx(watch.cost) + " " + watch.currency
                  }}</span>
                </p>
              </div>
              <div>
                <p style="margin-left: 100px;">
                  利润：
                  <span
                    v-if="watch.isNoRate == 1"
                    :style="{
                      color:
                        watch.sellMoney - watch.cost > 0 ? '#0c7063' : 'red',
                    }"
                    >{{
                      formatNumberRgx(watch.sellMoney - watch.cost) +
                        " " +
                        watch.currency
                    }}</span
                  >
                  <span
                    v-if="watch.isNoRate == 0"
                    :style="{
                      color:
                        watch.sellMoney - watch.adminSettleCheckCost > 0
                          ? '#0c7063'
                          : 'red',
                    }"
                    >{{
                      formatNumberRgx(
                        watch.sellMoney - watch.adminSettleCheckCost
                      ) +
                        " " +
                        watch.currency
                    }}</span
                ...
                <div style="margin-left: 100px;display: flex;">
                  ...
                  <div>
                    <input
                      class="check-cost-input"
                      v-model="singleBonus[index].adminSettleCheckCost"
      ...
      <div>
        <p>
          总实际成本：<span>{{
            formatNumberRgx(allCost) + " " + currency
          }}</span>
        </p>
        <p>
          总核算成本：<span
            :style="{ color: assignedCost == allCost ? '#0c7063' : 'red' }"
            >{{ formatNumberRgx(assignedCost) + " " + currency }}</span
          >
        </p>
      </div>
      <div>
        <p>
          总利润：<span>{{ formatNumberRgx(allProfit) + " " + currency }}</span>
        </p>
      </div>
      <div>
        <p>分红金额：</p>
        <div style="margin-left: 10px;">
          <div class="bonus-details-style">
            <p>资金：</p>
            <p>{{ fundBonusSum.toFixed(2) + " " + currency }}</p>
          </div>
          <div class="bonus-details-style">
            <p>公司：</p>
            <p>{{ componyBonusSum.toFixed(2) + " " + currency }}</p>
          </div>

          <div class="bonus-details-style">
            <p>{{ buyUserNick }}采购员：</p>
            <p>{{ purchaseBonusSum.toFixed(2) + " " + currency }}</p>
          </div>

          <div v-for="(sell, index) in sellFinalList" :key="index">
            <div class="bonus-details-style">
              <p>{{ sell.userNick }}销售员：</p>
              <p>{{ sell.money.toFixed(2) + " " + currency }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="unBonusSelect.select == 2" class="three">
      ...
      <div>
        <div
          class="watch-info-top"
          :style="{ color: watchInfoBonus.profit > 0 ? '#0c7063' : 'red' }"
        >
          {{ formatNumberRgx(watchInfoBonus.profit) }}
          <span> {{ watchInfoBonus.currency }}</span>
        </div>
        <div>
          <p>手表：</p>
          <div style="margin-left: 20px;">
            <p>
              品牌：<span>{{ watchInfoBonus.watch.watchBrand }}</span>
            </p>
            <p>
              型号：<span>{{ watchInfoBonus.watch.watchModel }}</span>
            </p>
            <p>
              等级：<span>{{ watchInfoBonus.watch.watchLevel }}</span>
            </p>
            <p>
              批发价：<span>{{
                formatNumberRgx(watchInfoBonus.watch.price) +
                  " " +
                  watchInfoBonus.watch.marketCurrency
              }}</span>
            </p>
          </div>
        </div>
        <div>
          ...
          <div style="margin-left: 20px;">
            <p>
              采购员：<span>{{ watchInfoBonus.purchase.buyUserNick }}</span>
            </p>
            <p>
              采购日期：<span>{{ watchInfoBonus.purchase.buyDate }}</span>
            </p>
            <p>
              机芯号：<span>{{ watchInfoBonus.purchase.buyWatchSn }}</span>
            </p>
            <p>
              采购价格：
              <span>{{
                formatNumberRgx(watchInfoBonus.purchase.buyWatchPrice) +
                  " " +
                  watchInfoBonus.purchase.buyWatchCurrency
              }}</span>
              <span
                >（
                {{
                  formatNumberRgx(watchInfoBonus.purchase.buyWatchPriceHkd) +
                    " " +
                    currency
                }}
                ）</span
            ...
            <p>
              应退税金额：
              <span>{{
                formatNumberRgx(watchInfoBonus.purchase.buyTaxMoney) +
                  " " +
                  watchInfoBonus.purchase.buyTaxCurrency
              }}</span>
            </p>
            <p>
              实退税金额：
              <span>{{
                formatNumberRgx(watchInfoBonus.purchase.buyTaxRecvMoney) +
                  " " +
                  watchInfoBonus.purchase.buyTaxRecvCurrency
              }}</span>
              <span
                >（
                {{
                  formatNumberRgx(watchInfoBonus.purchase.buyTaxRecvMoneyHkd) +
                    " " +
                    currency
                }}
                ）</span
            ...
            <p>
              小费：
              <span>{{
                watchInfoBonus.purchase.buyFeeMoney == 0
                  ? "无"
                  : formatNumberRgx(watchInfoBonus.purchase.buyFeeMoney) +
                    " " +
                    watchInfoBonus.purchase.buyWatchCurrency
              }}</span>
            </p>
            <p>
              佣金：
              <span>{{
                watchInfoBonus.purchase.buyCommMoney == 0
                  ? "无"
                  : formatNumberRgx(watchInfoBonus.purchase.buyCommMoney) +
                    " " +
                    watchInfoBonus.purchase.buyWatchCurrency
              }}</span>
            </p>
            <p>
              物流费：
              <span>{{
                formatNumberRgx(watchInfoBonus.purchase.logMoney) +
                  " " +
                  watchInfoBonus.currency
              }}</span>
            </p>
            <p>
              合计成本：
              <span>{{
                formatNumberRgx(watchInfoBonus.purchase.cost) +
                  " " +
                  watchInfoBonus.currency
              }}</span>
            </p>
        ...
        <div>
          ...
          <div style="margin-left: 20px;">
            <p>
              销售员：<span>{{ watchInfoBonus.sellUserNick }}</span>
            </p>
            <p>
              销售金额：<span>{{
                formatNumberRgx(watchInfoBonus.sellMoney) +
                  " " +
                  watchInfoBonus.currency
              }}</span>
            </p>
        ...
        <div>
          ...
          <div style="margin-left: 20px;">
            <p>
              核算成本：
              <el-input
                v-model="watchList[checkIndex].adminSettleCheckCost"
                style="width: 300px;height: auto;line-height: 40px;"
                @input="checkCostChange"
              >
                <i
                  slot="suffix"
                  style="color: #000;margin-right:5%;font-style:normal;"
                  >{{ watchInfoBonus.currency }}</i
                >
              </el-input>
            </p>
            <div style="width: 60%;">
              ...
              <div class="bonus-style">
                <p>
                  资金：{{
                    watchList[checkIndex].fundBonusMoney +
                      " " +
                      watchInfoBonus.currency
                  }}
                </p>
                <p>
                  <el-input
                    v-model="watchList[checkIndex].fundRecvBonusMoney"
                    @input="bonusChange"
              ...
              <div class="bonus-style">
                <p>
                  公司：{{
                    watchList[checkIndex].companyBonusMoney +
                      " " +
                      watchInfoBonus.currency
                  }}
                </p>
                <p>
                  <el-input
                    v-model="watchList[checkIndex].companyRecvBonusMoney"
                    @input="bonusChange"
                    style="width: 300px;height: 35px;line-height: 35px;"
                  >
                  </el-input>
              ...
              <div class="bonus-style">
                <p>
                  采购：{{
                    watchList[checkIndex].buyBonusMoney +
                      " " +
                      watchInfoBonus.currency
                  }}
                </p>
                <p>
                  <el-input
                    v-model="watchList[checkIndex].buyRecvBonusMoney"
                    @input="bonusChange"
                    style="width: 300px;height: 35px;line-height: 35px;"
                  >
                  </el-input>
              ...
              <div class="bonus-style">
                <p>
                  销售：{{
                    watchList[checkIndex].sellBonusMoney +
                      " " +
                      watchInfoBonus.currency
                  }}
                </p>
                <p>
                  <el-input
                    v-model="watchList[checkIndex].sellRecvBonusMoney"
                    @input="bonusChange"
                    style="width: 300px;height: 35px;line-height: 35px;"
                  >
                  </el-input>
              ...
              <div class="bonus-style">
                <p>
                  合计：<span style="color: #0c7063;">{{
                    bonusMoneyWatch + " " + watchInfoBonus.currency
                  }}</span>
                </p>
                <p
                  :style="{
                    color: bonusMoneyAll == bonusMoneyWatch ? '#0c7063' : 'red',
                  }"
                >
                  {{ bonusMoneyAll }}
                </p>
        ...
        <div style="width: 300px;margin: 0 auto;margin-top: 30px;">
          <el-button type="primary" style="width: 100%;" @click="BonusSure"
            >确 认</el-button
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    disposeBonus(userId, nick) {
      ...
      this.$axios
        .post(this.$store.state.baseUrl + "/unBonusList?java", {
      ...
      for (let j = 0; j < this.watchList.length; j++) {
        for (let i = 0; i < this.singleBonus.length; i++) {
          ...
          if (this.watchList[j].id == this.singleBonus[i].id) {
            this.singleBonus[i].adminSettleCheckCost = this.watchList[
              j
        ...
        if (checkeds[this.checkIndex].checked == false) {
          ...
          for (let m = 0; m < this.watchList.length; m++) {
            if (this.watchList[m].id == id) {
              this.watchList[m].adminSettleCheckCost = this.singleBonus[
                index
              ].adminSettleCheckCost;
            }
          }
          ...
          for (let index = 0; index < this.watchList.length; index++) {
            ...
            this.assignedCost += Number(
              this.watchList[index].adminSettleCheckCost
            );
          }
          this.allProfit = 0;
          for (let j = 0; j < this.watchList.length; j++) {
            for (let i = 0; i < this.singleBonus.length; i++) {
              ...
              if (this.watchList[j].id == this.singleBonus[i].id) {
                this.allProfit +=
                  Number(this.singleBonus[i].sellMoney) -
                  Number(this.watchList[j].adminSettleCheckCost);
              }
            }
          }
            ...
            if (this.watchList[index].id == id) {
              ...
              this.$axios
                ...
                .then((res) => {
                  ...
                  this.watchInfoBonus.profit =
                    this.watchInfoBonus.sellMoney -
                    this.watchList[this.checkIndex].adminSettleCheckCost;
                  console.log("qqqqqqqqqqqqqqqqqqqqq");
                  console.log(this.watchList[this.checkIndex]);
                  this.bonusMoneyWatch = Number(
                    Number(this.watchList[this.checkIndex].fundRecvBonusMoney) +
                      Number(
                        this.watchList[this.checkIndex].companyRecvBonusMoney
                      ) +
                      Number(
                        this.watchList[this.checkIndex].buyRecvBonusMoney
                      ) +
                      Number(this.watchList[this.checkIndex].sellRecvBonusMoney)
                  ).toFixed(2);
                  this.bonusAll();
                  console.log("核算该手表");
                  this.watchFormerList = JSON.parse(
    ...
    checkCostChange() {
      console.log(this.watchFormerList);
      if (
        Number(this.watchInfoBonus.watch.price) -
          Number(this.watchList[this.checkIndex].adminSettleCheckCost) <
        0
      ) {
        ...
        this.watchList[this.checkIndex].companyBonusMoney = (
          (this.watchInfoBonus.sellMoney -
            Number(this.watchList[this.checkIndex].adminSettleCheckCost)) *
          0.2
        ).toFixed(2);
        ...
        if (this.watchInfoBonus.watch.watchLevel == 1) {
          this.watchList[this.checkIndex].buyBonusMoney = (
            (this.watchInfoBonus.sellMoney -
              Number(this.watchList[this.checkIndex].adminSettleCheckCost)) *
            0.2
          ).toFixed(2);

          this.watchList[this.checkIndex].sellBonusMoney = (
            (this.watchInfoBonus.sellMoney -
              Number(this.watchList[this.checkIndex].adminSettleCheckCost)) *
            0.3
          ).toFixed(2);
          ...
          this.watchList[this.checkIndex].sellRecvBonusMoney = (
            (this.watchInfoBonus.sellMoney -
              Number(this.watchList[this.checkIndex].adminSettleCheckCost)) *
            0.3
          ).toFixed(2);
        ...
        Number(
          this.watchInfoBonus.sellMoney - this.watchInfoBonus.watch.price
        ) < 0
      ) {
        console.log("售价小于批发价");
        // this.watchList[this.checkIndex].fundBonusMoney = ((this.watchInfoBonus.sellMoney - Number(this
        ...
        this.watchList[this.checkIndex].companyBonusMoney = (
          (this.watchInfoBonus.watch.price -
            Number(this.watchList[this.checkIndex].adminSettleCheckCost)) *
            0.2 +
          (this.watchInfoBonus.sellMoney - this.watchInfoBonus.watch.price) *
            0.2
      ...
      this.watchList[this.checkIndex].fundBonusMoney = (
        this.watchInfoBonus.sellMoney -
        Number(this.watchList[this.checkIndex].adminSettleCheckCost) -
        this.watchList[this.checkIndex].buyBonusMoney -
        this.watchList[this.checkIndex].sellBonusMoney -
      ...
      this.watchList[this.checkIndex].fundRecvBonusMoney = (
        this.watchInfoBonus.sellMoney -
        Number(this.watchList[this.checkIndex].adminSettleCheckCost) -
        this.watchList[this.checkIndex].buyRecvBonusMoney -
        this.watchList[this.checkIndex].sellRecvBonusMoney -
    ...
    bonusAll() {
      this.bonusMoneyWatch = Number(
        Number(this.watchList[this.checkIndex].fundBonusMoney) +
          Number(this.watchList[this.checkIndex].companyBonusMoney) +
          Number(this.watchList[this.checkIndex].buyBonusMoney) +
          Number(this.watchList[this.checkIndex].sellBonusMoney)
      ).toFixed(2);
      this.bonusMoneyAll = Number(
        Number(this.watchList[this.checkIndex].fundRecvBonusMoney) +
          Number(this.watchList[this.checkIndex].companyRecvBonusMoney) +
          Number(this.watchList[this.checkIndex].buyRecvBonusMoney) +
          Number(this.watchList[this.checkIndex].sellRecvBonusMoney)
      ).toFixed(2);
    },
      ...
      for (let i = 0; i < this.watchList.length; i++) {
        for (let j = 0; j < this.singleBonus.length; j++) {
          if (this.singleBonus[j].id == this.watchList[i].id) {
            ...
            console.log(
              Number(
                this.singleBonus[j].sellMoney - this.singleBonus[j].price
              ) > 0
            );
            if (
              Number(
                this.singleBonus[j].price -
                  this.watchList[i].adminSettleCheckCost
              ) < 0
            ) {
              console.log("批发价小于核算成本");
              ...
              this.watchList[i].companyBonusMoney = (
                (this.singleBonus[j].sellMoney -
                  this.watchList[i].adminSettleCheckCost) *
                0.2
              ).toFixed(2);
              ...
              if (this.singleBonus[j].watchLevel == 1) {
                this.watchList[i].buyRecvBonusMoney = (
                  (this.singleBonus[j].sellMoney -
                    this.watchList[i].adminSettleCheckCost) *
                  0.2
                ).toFixed(2);
                this.watchList[i].buyBonusMoney = (
                  (this.singleBonus[j].sellMoney -
                    this.watchList[i].adminSettleCheckCost) *
                  0.2
                ).toFixed(2);

                this.watchList[i].sellRecvBonusMoney = (
                  (this.singleBonus[j].sellMoney -
                    this.watchList[i].adminSettleCheckCost) *
                  0.3
                ).toFixed(2);
                this.watchList[i].sellBonusMoney = (
                  (this.singleBonus[j].sellMoney -
                    this.watchList[i].adminSettleCheckCost) *
                  0.3
                ).toFixed(2);
              ...
              this.watchList[i].companyRecvBonusMoney = (
                (this.singleBonus[j].price -
                  this.watchList[i].adminSettleCheckCost) *
                  0.2 +
                (this.singleBonus[j].sellMoney - this.singleBonus[i].price) *
                  0.2
              ).toFixed(2);
              this.watchList[i].companyBonusMoney = (
                (this.singleBonus[j].price -
                  this.watchList[i].adminSettleCheckCost) *
                  0.2 +
                (this.singleBonus[i].sellMoney - this.singleBonus[i].price) *
                  0.2
              ).toFixed(2);
            ...
            this.watchList[i].fundRecvBonusMoney = (
              this.singleBonus[j].sellMoney -
              this.watchList[i].adminSettleCheckCost -
              this.watchList[i].buyRecvBonusMoney -
              this.watchList[i].sellRecvBonusMoney -
            ...
            this.watchList[i].fundBonusMoney = (
              this.singleBonus[j].sellMoney -
              this.watchList[i].adminSettleCheckCost -
              this.watchList[i].buyBonusMoney -
              this.watchList[i].sellBonusMoney -
    ...
    bonusComputed(watchList) {
      ...
      for (let i = 0; i < watchList.length; i++) {
        ...
        this.fundBonusSum += Number(
          Number(watchList[i].fundRecvBonusMoney).toFixed(2)
        );
        this.componyBonusSum += Number(
          Number(watchList[i].companyRecvBonusMoney).toFixed(2)
        );
        this.purchaseBonusSum += Number(
          Number(watchList[i].buyRecvBonusMoney).toFixed(2)
        );
    ...
    checkedChange(e, watch, index) {
      ...
      if (e.target.checked == false && this.watchList.length == 1) {
        ...
        if (e.target.checked == true) {
          ...
          this.allCost += Number(watch.cost);
          console.log(
            Number(watch.sellMoney) -
              Number(this.singleBonus[index].adminSettleCheckCost)
          );
          this.allProfit +=
            Number(watch.sellMoney) -
            Number(this.singleBonus[index].adminSettleCheckCost);
    ...
    checkData() {
      for (let j = 0; j < this.watchList.length; j++) {
        for (let i = 0; i < this.singleBonus.length; i++) {
          if (this.watchList[j].id == this.singleBonus[i].id) {
            if (
              this.singleBonus[i].sellMoney -
                this.singleBonus[i].adminSettleCheckCost <
              0
            ) {
              this.$message.warning({
    ...
    generateBonusSureNotes() {
      ...
      console.log(this.users);
      this.$axios
        .post(this.$store.state.baseUrl + "/bonusFormSave?java", {
    ...
    gobackBonusWatch() {
      console.log(this.watchFormerList);
      this.watchList.splice(
        this.checkIndex,
        1,
        this.watchFormerList[this.checkIndex]
      );
      console.log(this.watchList);
      // this.calculateDividend();
      this.unBonusSelect.select = 1;
    ...
    getList() {
      ...
      this.list = this.unBonusList.filter(
        (item, index) =>
