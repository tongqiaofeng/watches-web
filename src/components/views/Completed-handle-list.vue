  ...
  <div>
    ...
    <div v-show="completedRecordList.length !== 0">
      ...
      <div v-for="(item,index) in list" :key="index" class="borrow-table">
        <img src="../../assets/imgs/pass.png" v-show="item.type == 2" class="passImg" />
        <table>
          <tr>
            <th>对象</th>
            <th>原因/描述</th>
          ...
          <tr>
            ...
            <td>{{item.time}}</td>
            <td>
              <p :style="{color: item.type == 1 || item.type == 5 ? '#0c7063' : (item.type == 2 ? '#aaa' : 'red')}">
                {{item.type == 1 || item.type == 5 ? '+' +formatNumberRgx(item.money) + ' ' +item.currency : (item.type == 2 ? formatNumberRgx(item.money) + ' ' +item.currency : '-' +formatNumberRgx(item.money) + ' ' +item.currency)}}
              </p>
            </td>
            <td>
