  ...
  <div class="reimbursement-container" id="reimbursement">
    ...
    <div v-if="reimbursementSel.select == 0">
      <div class="reimbursement-container-center">
        ...
        <el-button type="info" style="width: 160px;height: 45px;font-size: 14px;" @click="addNotJourney">新增非行程消费
        </el-button>
        ...
        <el-tab-pane label="新增消费单" name="first">
          <div style="display: flex;justify-content: space-between">
            <div style="margin-bottom: 30px;margin-top: 10px;text-align: center;">
              <el-checkbox-group v-model="typeList" @change="filtrateMsgSure">
                      ...
                      <div style="margin-top: 20px;border-top: 1px solid #ddd;">
                        ...
                        <div style="padding-left: 15px;display: flex;justify-content: space-between;">
                          ...
                          <img src="../../assets/imgs/calc2.png" style="height: 35px;cursor: pointer;"
                            @click="rateCalcJump(12,unJourneyInfo.rateTime,1)" />
                        </div>
                        ...
                        <p style="padding-left: 15px;">
                          ...
                          预估报销金额：<span>{{formatNumberRgx(unJourneyInfo.sysCheckMoney) + ' ' + unJourneyInfo.settleCurrency}}</span>
                        </p>
                        <p style="padding-left: 15px;">
                          核算报销金额：<span>{{formatNumberRgx(unJourneyInfo.checkMoney) + ' ' + unJourneyInfo.settleCurrency}}</span>
                        </p>
                      </div>
                    </div>
                    <div slot="footer">
          ...
          <div id="activeSecond">
            <el-tabs type="border-card" v-model="activeSecondName" @tab-click="handleSecondTabClick">
              ...
              <el-tab-pane name="three">
                ...
                <div>
                  <To-confirmed v-if="activeSecondName == 'three'" :toConfirmedSel="toConfirmedSel" @tojump='tojump'
                    @deleteReco2="deleteReco2" @rateCalcJump="rateCalcJump" @sumCom="sumCom"></To-confirmed>
                </div>
              </el-tab-pane>
              <el-tab-pane name="four">
                <div slot="label" style="font-size: 15px;">已完成</div>
                <div>
                  <Complete-reimbursement v-if="activeSecondName == 'four'" @tojump2='tojump2'
                    @deleteReco3="deleteReco3" :completeReimbursementSel="completeReimbursementSel"
      ...
      <div class="flagTable">
        ...
        <div>
          <div style="margin-top: 15px;">
            ...
            <span>{{formatNumberRgx(sysCheckMoney) + ' ' + settleCurrency}}</span>
          </div>
          <p style="margin: 0;font-size: 14px;color: #bbb;">实际报销费用以财务核算为准</p>
      ...
      <div class="flagTable">
          ...
          <table>
            ...
            <tr v-for="(every,index) in journeyInfo.recordList" :key="index">
              ...
              <td>
                ...
                {{formatNumberRgx(every.sysCheckMoney) + ' ' + journeyInfo.settleCurrency}}
              </td>
              <td>
                {{formatNumberRgx(every.checkMoney) + ' ' + journeyInfo.settleCurrency}}
              </td>
              <td>{{every.time}}</td>
              <td>
                <el-tooltip class="item" effect="light" content="查看详情" placement="top-end">
                  ...
                  <div>
                    ...
                    <div style="margin-top: 20px;border-top: 1px solid #ddd;">
                      ...
                      <div style="padding-left: 15px;display: flex;justify-content: space-between;">
                        ...
                        <img src="../../assets/imgs/calc2.png" style="height: 35px;cursor: pointer;"
                          @click="rateCalcJump(12,recordDetailsMsg.rateTime,1)" />
                      </div>
                      <p style="margin-top: 0;padding-left: 15px;">
                      ...
                      <p style="padding-left: 15px;">
                        ...
                        预估报销金额：<span>{{formatNumberRgx(recordDetailsMsg.sysCheckMoney) + ' ' + journeyInfo.settleCurrency}}</span>
                      </p>
                      <p style="padding-left: 15px;">
                        核算报销金额：<span>{{formatNumberRgx(recordDetailsMsg.checkMoney) + ' ' + journeyInfo.settleCurrency}}</span>
                      </p>
                    </div>
                  </div>
                  <div slot="footer">
        ...
        <div>
          ...
          <p style="margin-left: 20px;">
            ...
            预估报销金额：<span>{{formatNumberRgx(journeyInfo.sysCheckMoney) + ' ' + journeyInfo.settleCurrency}}</span>
          </p>
          <p style="margin-left: 20px;">
            核算报销金额：<span>{{formatNumberRgx(journeyInfo.checkMoney) + ' ' + journeyInfo.settleCurrency}}</span>
          </p>
        </div>
      </div>
...
<script>
  export default {
    ...
    props: ['reimbursementSel', 'jumpToRate'],
    created() {
      this.handleCountryGet();
    },
    mounted() {
      if (this.jumpToRate.selectval == 12) { //判断是否是汇率页面跳转回来的数据
        this.activeName = sessionStorage.getItem('setactiveName2');
        this.activeSecondName = sessionStorage.getItem('activeSecondName');
        this.typeList = JSON.parse(sessionStorage.getItem('typeListSel'));
        this.page = sessionStorage.getItem('reimPage');
        this.page = Number(this.page);
        this.startTime = sessionStorage.getItem('startTimeJump');
        this.endTime = sessionStorage.getItem('endTimeJump');
        this.handleReimburseList()
        if (sessionStorage.getItem('reimbursementSelselect') == 0) {
          if (this.activeName === 'first') {
            // 非行程记录信息 
            console.log("fff");
            this.updateReimburse(sessionStorage.getItem('2type'), sessionStorage.getItem('2flag'), sessionStorage
              .getItem('2id'));
          }
          if (this.activeSecondName == 'three') {
            this.toConfirmedSel.val = this.jumpToRate.selectval //three
            if ((sessionStorage.getItem('tojumpval')) == 2) {
              this.toConfirmedSel.select = 2; //记录详情
              console.log(sessionStorage.getItem('tojumpval1'));
            } else {
              this.toConfirmedSel.select = 1; //非行程记录信息
              console.log(sessionStorage.getItem('tojumpval2'));
            }
          } else if (this.activeSecondName == 'four') {
            this.completeReimbursementSel.val = this.jumpToRate.selectval //four
            if ((sessionStorage.getItem('tojumpval2')) == 2) {
              this.completeReimbursementSel.select = 2; //记录详情
              console.log(sessionStorage.getItem('tojumpval3'));
            } else {
              this.completeReimbursementSel.select = 1; //非行程记录信息
              console.log(sessionStorage.getItem('tojumpval4'));
            }
          }
        } else if (sessionStorage.getItem('reimbursementSelselect') == 4) {
          // 记录信息
          this.updateReimburse(sessionStorage.getItem('2type'), sessionStorage.getItem('2flag'), sessionStorage.getItem(
            '2id'))
          this.recordDetails(JSON.parse(sessionStorage.getItem('2recorevery')))
        }
      } else {
        this.handleReimburseList();
      }
    ...
    methods: {
      ...
      deleteReco2() {
        ...
        this.toConfirmedSel.val = '';
      },
      deleteReco3() {
        this.jumpToRate.selectval = '';
        this.completeReimbursementSel.val = '';
      },
      isPdf(img) {
        // console.log(img);
        if (img !== '' && img !== null) {
      ...
      rateCalcJump(val, time, page) {
        ...
        // console.log(sessionStorage.getItem('reimbursementselect'));
        sessionStorage.setItem('reimbursementSelselect', this.reimbursementSel.select)
        console.log(this.reimbursementSel.select);
        sessionStorage.setItem('setactiveName2', this.activeName)
        sessionStorage.setItem('activeSecondName', this.activeSecondName)
        console.log(this.activeName + "------" + this.activeSecondName);
        sessionStorage.setItem('typeListSel', JSON.stringify(this.typeList));
        sessionStorage.setItem('reimPage', this.page);
        sessionStorage.setItem('startTimeJump', this.startTime);
        sessionStorage.setItem('endTimeJump', this.endTime);
        this.$emit('jumpRate', 12, time, 1);
      },
      tojump(val) {
        console.log(val);
        sessionStorage.setItem('tojumpval', val)
        //判断three table中行程与非行程页面
      },
      tojump2(val) {
        console.log(val);
        sessionStorage.setItem('tojumpval2', val)
        //four table中行程与非行程页面
      },
      // 获取是否有需要确认的报销单
      ...
      updateReimburse(type, flag, id) {
        sessionStorage.setItem('2type', type)
        sessionStorage.setItem('2flag', flag)
        sessionStorage.setItem('2id', id)
        console.log('修改');
        ...
        if (this.updateType == 0) {
          ...
          this.$axios.post(this.$store.state.baseUrl + '/TripInfo?java', {
            ...
            this.settleCurrency = res.data.settleCurrency;
      ...
      recordDetails(every) {
        ...
        sessionStorage.setItem('2recorevery', JSON.stringify(every))
