<template>
  <div>
    ...
    <div
      v-show="detailsList.length > 1"
      id="myChart"
      style="width: 80%;height: 400px;margin: 30px auto;"
    ></div>
    <div
      style="width: 50%;margin: 30px auto;display: flex;"
      v-show="marketPriceAuthority[11] == 1"
    >
    ...
    <div class="details-bottom" v-if="detailsList.length !== 0">
      ...
      >
        ...
        <div>
          <span>最低：</span>
          <span>{{
            companyLoginCurrency + " " + formatNumberRgx(priceList[0])
          }}</span>
        </div>
      </div>
      <table>
        <tr>
          ...
          <th>趋势</th>
          <th>更新时间</th>
          <th v-show="marketPriceAuthority[11] == 1">操作</th>
        </tr>
        <tr v-for="(item, index) in detailsList" :key="index">
          <td>
            {{ companyLoginCurrency + " " + formatNumberRgx(item.price) }}
          </td>
          <td>
          ...
          <td>{{ item.time }}</td>
          <td v-show="marketPriceAuthority[11] == 1">
            <el-tooltip
              class="item"
              effect="light"
              content="删除信息"
            ...
            >
              <img
                ...
                @click="deletPrice(item.id)"
              />
            </el-tooltip>
            <el-dialog
              title="删除批发价格"
              :visible.sync="dialogDeletPriceVisible"
            >
              <div style="text-align: center;">
                <p>确定删除该批发价格？删除后不可恢复</p>
              </div>
              <div slot="footer">
                <el-button @click="dialogDeletPriceVisible = false"
                  >取 消</el-button
                >
                <el-button type="primary" @click="deletPriceSure" v-preventClick
                  >确 定</el-button
                >
              </div>
            </el-dialog>
...
export default {
  ...
  created() {
    ...
    this.marketPriceAuthority = sessionStorage.getItem("authority").split("|");
    this.watchId = this.priceDetailsList.buyWatchId;
    if (this.priceDetailsList.prices == null) {
      this.detailsList = [];
    } else {
  ...
  methods: {
    ...
    priceAdd() {
      if (this.price == "") {
        ...
        this.$axios
          .post(this.$store.state.baseUrl + "/DataMaketPriceAdd", {
            id: this.watchId,
            price: this.price,
            currency: this.companyLoginCurrency,
          })
          .then((res) => {
            console.log("增加批发价格");
            console.log(res);
    ...
    deletPriceSure() {
      this.$axios
        .post(this.$store.state.baseUrl + "/DataMaketPriceDel", {
          id: this.priceId,
