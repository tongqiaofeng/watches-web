<template>
  <div class="Sale-reports" id="Sale-reports">
          ...
          <el-form id="report-input" ref="form">
            <div class="report-input">
              ...
              <el-form-item
                label="用户列表:"
                style="width: 50%; "
                :rules="[{ required: true, message: '用户列表不能为空' }]"
              >
                <el-select
                  v-model="value2"
                  filterable
          ...
          <div class="watch-nums">
            <div class="watch-salenums">
              ...
              <p class="watch-nums-total">
                {{
                  formatNumberRgx(salesreportList.allMoney) +
                    " " +
                    " " +
                    salesreportList.currency
                }}
              </p>
            </div>
          </div>
        </div>
        <div v-if="imgurl.length == 0" style="text-align: center;">
          <p>{{ hintMsg }}</p>
        </div>
          ...
          <table>
            ...
            <tr v-for="(item, index) in imgurl" :key="index" class="imgurlbox">
              ...
              <td style="text-align: left;">
                <div>
                  <span>品牌 ：</span>
                  <span>{{ item.watchBrand }}</span>
                </div>
                <div>
                  <span>型号 ：</span>
                  <span>{{ item.watchModel }}</span>
                </div>
                <div v-if="item.costMoney != 0">
                  <span>成本价：</span>
                  <span>{{
                    formatNumberRgx(item.costMoney) + "  " + comCurrency
                  }}</span>
                </div>
                <div v-else>
                  <span>采购价 ：</span>
                  <span>{{
                    formatNumberRgx(item.buyWatchPrice) + "  " + comCurrency
                  }}</span>
                </div>
                <div>
                  <span>销售价格 ：</span>
                  <span>{{
                    formatNumberRgx(item.sellMoney) + "  " + comCurrency
                  }}</span>
                </div>
                <div v-if="item.costMoney != 0">
                  <span>利润 ：</span>
                  <span
                    :style="
                      item.sellMoney - item.costMoney > 0
                        ? 'color:green;'
                        : 'color:red;'
                    "
                    >{{
                      formatNumberRgx(item.sellMoney - item.costMoney) +
                        "  " +
                        comCurrency
                    }}</span
                  >
    ...
    <div class="imgzhu" v-if="againclick.times == 1">
        ...
        <div class="message-content">
          ...
          <div class="message-main">
            <div class="message-mainbox">
              ...
              >
                <img
                  ...
                  class="messagelogo"
                /><span>品牌 ：</span
                ><span>{{ sellinfoList.buyWatchBrand }}</span>
              </div>
              <div
                style="display: flex;flex-wrap: nowrap;"
                class="mainbox-line"
              >
                <img
                  ...
                  class="messagelogo"
                /><span>型号 ：</span
                ><span>{{ sellinfoList.buyWatchModel }}</span>
              </div>
            </div>
            <div class="message-mainbox">
              <div
              ...
              >
                <img
                  src="../../assets/imgs/users.png"
                  alt=""
                  class="messagelogo"
                /><span>采购员 ：</span
                ><span>{{ sellmessage.buyUserNick }}</span>
              </div>
              <div
                style="display: flex;flex-wrap: nowrap;"
                class="mainbox-line"
              >
                ...
                />
                <span>销售员 ：</span
                ><span>{{ sellmessage.sellUserNick }}</span>
              </div>
              <div
                style="display: flex;flex-wrap: nowrap;"
                class="mainbox-line"
              >
                <img
                  src="../../assets/imgs/peoples.png"
                  alt=""
                  class="messagelogo"
                />
                <span>客户名称 ：</span
                ><span>{{ sellinfoList.sellCustom }}</span>
              </div>
            </div>
            <div class="message-mainbox">
              <div
              ...
              >
                ...
                />
                <span>入库时间 ：</span
                ><span v-if="sellmessage.stockInTime != null">{{
                  sellmessage.stockInTime
                }}</span
              ...
              >
                <img
                  ...
                  class="messagelogo"
                />
                <span>销售时间 ：</span><span>{{ sellinfoList.sellTime }}</span>
              </div>
            </div>
            <div class="message-mainbox">
              <div
            ...
            <div class="message-mainbox">
              ...
              >
                <img
                  src="../../assets/imgs/price.png"
                  alt=""
                  class="messagelogo"
                />
                <span>采购价格 ：</span
                ><span v-if="sellmessage.buyWatchPrice != 0">{{
                  formatNumberRgx(sellmessage.buyWatchPrice) +
                    " " +
                    sellmessage.currency
                }}</span
                ><span v-else> ---</span>
              </div>
              <div
              ...
              >
                ...
                />
                <span>成本价格 ： </span
                ><span v-if="sellmessage.costMoney != 0">{{
                  formatNumberRgx(sellmessage.costMoney) +
                    " " +
                    sellmessage.currency
                }}</span
                ><span v-else> ---</span>
              ...
              >
                ...
                />
                <span>批发价格 ：</span
                ><span v-if="sellmessage.marketPrice != 0">{{
                  formatNumberRgx(sellmessage.marketPrice) +
                    " " +
                    sellmessage.marketCurrency
                }}</span
                ><span v-else> ---</span>
              </div>
              <div
              ...
              >
                ...
                />
                <span>销售价格 ：</span
                ><span v-if="sellmessage.sellMoney != 0">{{
                  formatNumberRgx(sellmessage.sellMoney) +
                    " " +
                    sellmessage.sellCurrency
                }}</span
                ><span v-else> ---</span>
              </div>
              <div
              ...
              >
                ...
                ><span
                  :style="
                    sellmessage.sellMoney - sellmessage.costMoney > 0
                      ? 'color:green;'
                      : 'color:red;'
                  "
                  v-if="sellmessage.costMoney != 0"
                  >{{
                    formatNumberRgx(
                      sellmessage.sellMoney - sellmessage.costMoney
                    ) +
                      " " +
                      sellmessage.currency
                  }}</span
    ...
    <Details
      ...
      @backbuy="backbuy"
      v-if="againclick.times == 2"
    >
    </Details>
    <!--销售报表详情-->
    <Sale-details
      v-if="againclick.times == 4"
      @backbuy="backbuy"
    ...
    salesList() {
      this.$axios
        ...
        .then((res) => {
          ...
          this.value2 = this.salesLists[0].name;
          this.datanews.nick = this.value2;
          if (this.flag == 1) {
            this.purchaserLists = res.data.purchaserList;
            this.value1 = this.purchaserLists[0].name;
          }
        });
    },
    getDateTime() {
    ...
    sellOrder() {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          if (
            this.sellmessage.stockOutTime == "" ||
            this.sellmessage.stockOutTime == null
          ) {
